function [fig_handle] = display_dqmc_fit(fit_struct, mu_struct, fig_handle)
...
Title = sprintf('T = %0.2f(%.0f) t, U = %0.2f(%.0f), mu peak = %0.2f (mu=0 is n=1), h = %0.2f(%.0f) \n Img Fidelity = %0.2f(%.0f), Dbl Fidelity = %0.2f(%.0f), Fit densities n = %0.2f - %0.2f\n expt densities corrected for efficiency',...
    ...
    fit_struct.fit_params(5), fit_struct.std_errs(5) * 100,...
...
plot(fit_struct.n_interp, fit_struct.nd_fit_maxU, 'r-.');
ph4 = plot(fit_struct.n_interp, fit_struct.nd_fit_minU, 'r-.');
ph5 = plot(fit_struct.n_interp, fit_struct.nd_fg, 'g');
xlabel('<n>')
ylabel('<d>');
title(sprintf('doubles (fit = %d)', fit_struct.use_param_to_fit(1)));
grid on;
ylim([0, 1]);
legend([ph1, ph2, ph3, ph4, ph5], {'expt', 'dqmc' , 'T err', 'U err', 'NonIntFG'})
...
plot(fit_struct.n_interp, fit_struct.ns_fit_maxU, 'r-.');
ph4 = plot(fit_struct.n_interp, fit_struct.ns_fit_minU, 'r-.');
ph5 = plot(fit_struct.n_interp, fit_struct.ns_fg,'g');
xlabel('<n>')
ylabel('<n^s>');
...
plot(fit_struct.n_interp, fit_struct.nup_corr_fit_maxU, 'r-.');
plot(fit_struct.n_interp, fit_struct.nup_corr_fit_minU, 'r-.');
ph5 = plot(fit_struct.n_interp, fit_struct.nup_corr_fg, 'g');

xlabel('<n>');
ylabel('Corr(0,1)');
title(sprintf('up corr (fit = %d)', fit_struct.use_param_to_fit(3)));
grid on;
legend([ph1, ph2, ph5], {'up corr', 'dqmc', 'fg'});
ylim([-0.065, 0.015]);
...
plot(fit_struct.n_interp, fit_struct.ndn_corr_fit_maxU, 'r-.');
plot(fit_struct.n_interp, fit_struct.ndn_corr_fit_minU, 'r-.');
%fg
ph5 = plot(fit_struct.n_interp, fit_struct.ndn_corr_fg, 'g');
legend([ph1, ph2, ph5], {'down corr', 'dqmc', 'fg'});
xlabel('<n>');
ylabel('Corr(0,1)');
...
plot(fit_struct.n_interp, fit_struct.nd_corr_fit_maxU, 'r-.');
plot(fit_struct.n_interp, fit_struct.nd_corr_fit_minU, 'r-.');
ph5 = plot(fit_struct.n_interp, fit_struct.nd_corr_fg, 'g');

xlabel('<n>');
ylabel('Corr(0,1)');
title(sprintf('doubles corr (fit = %d)', fit_struct.use_param_to_fit(4)));
grid on;
legend([ph1, ph2, ph5], {'Expt', 'dqmc', 'fg'});
ylim([-0.065, 0.015]);
...
ph4 = plot(fit_struct.n_interp, fit_struct.ns_corr_fit_maxU, 'r-.');
ph5 = plot(fit_struct.n_interp, fit_struct.ns_corr_fit_minU, 'r-.');
ph6 = plot(fit_struct.n_interp, fit_struct.ns_corr_fg, 'g');

xlabel('<n>');
ylabel('Corr(0,1)');
title(sprintf( 'singles corr (fit = %d)', fit_struct.use_param_to_fit(5)) );
grid on;
legend([ph1, ph2, ph6], {'expt', 'dqmc', 'fg'});
ylim([-0.065, 0.015]);
...
grid on;
ylim([0, 1]);
legend([ph1, ph2, ph3, ph4], {'nup', 'nup dqmc', 'ndn', 'ndn dqmc'})
