function [V,PNames,ArgStr] = atomicLimitRadial_SinglesDensity1D(P,X)
...
%     Beta = P(1); U = P(2); Omega = P(3); Mu = P(4); A = P(5);
%     Pot = @(r) 0.5*mLi*Omega^2*r.^2;
%     Zg = @(r) (1+2*exp(Beta*Mu).*exp(-Beta*Pot(r))+exp(2*Beta*Mu).*exp(-Beta*(-U+2*Pot(r))));
%     V = A*(exp(Beta*Mu)*exp(-Beta*Pot(X))+ 2*exp(2*Beta*Mu)*exp(-Beta*(-U+2*Pot(X))))./Zg(X);
    ...
    V = A*(2*exp(Beta*(AvgMu+0.5)).*cosh(Beta*DeltaMu).*exp(-Beta*Pot(X)))./Zg(X);
end
end
